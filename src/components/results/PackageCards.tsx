...
export function PackageCards({ packages }: PackageCardsProps) {
  return (
    <SimpleGrid cols={{ base: 1, sm: 2, lg: 3 }} spacing="lg">
      {packages.map((pkg) => (
        <Card key={pkg.name} shadow="sm" padding="lg" withBorder>
          <Stack gap="md">
            ...
            <Group gap="xl">
              <Stack gap={4}>
                <Text size="xs" c="dimmed">Weekly Downloads</Text>
                <Text size="lg" fw={600}>
                  {formatNumber(pkg.weeklyDownloads)}
                </Text>
              </Stack>
              <Stack gap={4}>
                <Text size="xs" c="dimmed">GitHub Stars</Text>
                <Text size="lg" fw={600}>
                  {formatNumber(pkg.stars)}
                </Text>
              </Stack>
              <Stack gap={4}>
                <Text size="xs" c="dimmed">Forks</Text>
                <Text size="lg" fw={600}>
